...
docker run --rm -d \
  ...
  --mount type=bind,source="$(pwd)"/config/edge/default.conf,target=/etc/nginx/conf.d/default.conf \
  --mount type=bind,source="$(pwd)"/core,target=/var/www/html \
  nginx:1.23-alpine  
```
...
Также стоит обратить внимание на конфигурационный файл Nginx [default.conf](lemp/config/edge/default.conf). В нем указано слушать 80-й порт и все запросы, которые в адресе содержат расширение `.php` перенаправлять в контейнер `php-fpm` на 9000-й порт:
